...
export default function Dealer({}) {
  const [dealerView, setDealerView] = useState("list");
    ...
    <>
      <div className="p-2 bg-gray-50">
        ...
        {dealerView === "map" ? (
          <div id="map-view">
            <img
              src="https://ccsitebuild.vercel.app/_next/image?url=https%3A%2F%2Fwww.google.com%2Fmaps%2Fd%2Fthumbnail%3Fmid%3D1PnZbJHczAGCMhrJalaRQ8AcKnS8%26hl%3Den&w=3840&q=75"
              className="w-full h-full"
              alt=""
            />
          </div>
        ) : (
          <ul className="list-unstyled flex flex-col gap-1" id="list-view">
            {dealers.map((block, i) => {
              return (
        ...
        <ul className="grid grid-cols-2 align-items-center gap-1 mt-1">
          <li className="">
            <button
              className={`p-1 flex text-nowrap items-center gap-1 w-full justify-center ${
                dealerView === "list" ? "bg-[#ffc20f]" : "bg-[#e4e4e4]"
              }`}
              onClick={(e) => {
                e.preventDefault();
                setDealerView("list");
          ...
          <li>
            <button
              className={`p-1 flex text-nowrap items-center gap-1 w-full justify-center ${
                dealerView === "map" ? "bg-[#ffc20f]" : "bg-[#e4e4e4]"
              }`}
              onClick={(e) => {
                e.preventDefault();
                setDealerView("map");
